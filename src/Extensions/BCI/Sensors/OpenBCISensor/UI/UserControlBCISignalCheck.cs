...
namespace ACAT.Extensions.BCI.Sensors.OpenBCISensor
{
    ...
    /// Displays signals from electrodes and does railing and impedance tests
    /// </summary>
    public partial class UserControlBCISignalCheck : UserControlBCIBase
    {
        ...
        public UserControlBCISignalCheck(String stepId)
        {
            ...
            _signalQualityGradientImages[0] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_1AcceptableChannel; // for heatmap - 0 is the same as 1 accepted channel
            _signalQualityGradientImages[1] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_1AcceptableChannel;
            _signalQualityGradientImages[2] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_2AcceptableChannels;
            _signalQualityGradientImages[3] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_3AcceptableChannels;
            _signalQualityGradientImages[4] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_4AcceptableChannels;
            _signalQualityGradientImages[5] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_5AcceptableChannels;
            _signalQualityGradientImages[6] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_6AcceptableChannels;
            _signalQualityGradientImages[7] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_7AcceptableChannels;
            _signalQualityGradientImages[8] = global::openBCISensorUI.Properties.Resources.signalQualityGradient_8AcceptableChannels;
        ...
        public void resetSavedSignalQualityValues()
        {
            ...
            OpenBCISensorSettings.Settings.SignalControl_RecheckNeeded = true;
            OpenBCISensorSettings.Settings.SignalQuality_PassedLastOverallQualityCheck = false;
            OpenBCISensorSettings.Save();
        }
        ...
        public OverallSignalQualityResult getElectrodeSignalQualityResults()
        {
            ...
            for (int chIdx = 0; chIdx < _numChannels; chIdx++)
            {
                ...
                if (OpenBCISensorSettings.Settings.SignalQuality_AcceptanceMode == "Top8" && eegChannel.isRequiredElectrode)
                {
                ...
                else if (OpenBCISensorSettings.Settings.SignalQuality_AcceptanceMode == "AllEnabled" || OpenBCISensorSettings.Settings.SignalQuality_AcceptanceMode != "Top8")
                {
                    ...
                    if (chIdx >= 8 && OpenBCISensorSettings.Settings.DAQ_NumEEGChannels != 16)
            ...
            else
            {
                ...
                if (newResult.numOverallGoodElectrodes >= OpenBCISensorSettings.Settings.SignalQuality_MinOverallGoodChannels &&
                    newResult.numOverallOkElectrodes <= OpenBCISensorSettings.Settings.SignalQuality_MaxOverallOKChannels​ &&
                    newResult.numOverallBadElectrodes <= OpenBCISensorSettings.Settings.SignalQuality_MaxOverallBadChannels​)
                {
                    // All electrodes have valid values (none have status Error) and overall signal quality criteria met
                    OpenBCISensorSettings.Settings.SignalQuality_PassedLastOverallQualityCheck = true;
                }
                else
                {
                    OpenBCISensorSettings.Settings.SignalQuality_PassedLastOverallQualityCheck = false;
            ...
            bool userPassedLastSignalQualityCheck)
        {
            ...
            _Testing_BCIOnboardingIgnoreOpticalSensorChecks = OpenBCISensorSettings.Settings.Testing_BCIOnboardingIgnoreOpticalSensorChecks;

            // Get / initialize variables related to board config used in data processing
            _indEegChannels = DAQ_OpenBCI.indEegChannels;
            _numChannels = OpenBCISensorSettings.Settings.DAQ_NumEEGChannels;
            _samplingRate = DAQ_OpenBCI.sampleRate;
            _scaleIdx = OpenBCISensorSettings.Settings.SignalMonitor_ScaleIdx;
            ...
            if (OpenBCISensorSettings.Settings.Testing_DuplicateRequiredChannelsAsOptionalChannels)
                _numChannels = 16;
            ...
            new List<String> { OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel1_Name,OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel2_Name,
                OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel3_Name,OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel4_Name,
                OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel5_Name,OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel6_Name,
                OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel7_Name,OpenBCISensorSettings.Settings.SignalControl_RequiredChannel_Channel8_Name};

            _channelNamesOptional =
            new List<String> { OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel9_Name, OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel10_Name,
                OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel11_Name, OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel12_Name,
                OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel13_Name, OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel14_Name,
                OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel15_Name, OpenBCISensorSettings.Settings.SignalControl_OptionalChannel_Channel16_Name};
            ...
            for (int chIdx = 0; chIdx < _numChannels; chIdx++)
            {
                ...
                bool channelEnabled = OpenBCISensorSettings.Settings.GetClassifier_EnableChannel(chIdx);
        ...
        private void StartImpedanceTesting()
        {
            ...
            if (!_runImpedanceTestingCycle && DAQ_OpenBCI.deviceInitialized)
            {
                ...
                while (_runImpedanceTestingCycle)
                {
                    ...
                    {
                        _currentImpedanceTestElectrodeIndex = 0;
                        if (OpenBCISensorSettings.Settings.SignalQuality_StopImpedanceTestAfterOneCycle)
                        {
                ...
                if (_AllElectrodesOverallSignalQualityResult.allElectrodesUpdatedWithinSession == true)
                {
                    ...
                    OpenBCISensorSettings.Settings.SignalQuality_TimeOfLastImpedanceCheck = DateTimeOffset.Now.ToUnixTimeSeconds();
                    OpenBCISensorSettings.Save(); // Save settings
                }
        ...
        {
            if (OpenBCIDeviceTester._endSignalCheckTimer)
                return;
        ...
        private void updateRailingTestResult(int chIdx, int railingResultPercentage, bool update_UI)
        {
            try
            {
                OpenBCISensorSettings.Settings.SignalQuality_LastRailingValues[chIdx] = railingResultPercentage;
                ...
                if (update_UI && railingResultPercentage != int.MaxValue)
                {
                    Invoke(new Action(() =>
                    {
                        ...
                        if (railingResultPercentage <= OpenBCISensorSettings.Settings.SignalQuality_RailingGoodMaxThreshold)
                            railingResultColor = SelectColorFromStatus(SignalStatus.SIGNAL_OK);
                        else if (railingResultPercentage > OpenBCISensorSettings.Settings.SignalQuality_RailingGoodMaxThreshold &&
                            railingResultPercentage <= OpenBCISensorSettings.Settings.SignalQuality_RailingOkMaxThreshold​)
        ...
        private void updateImpedanceResult(int chIdx, int impedanceResult, bool update_ui, bool loadedSavedValue)
        {
            try
            {
                OpenBCISensorSettings.Settings.SignalQuality_LastImpedanceValues[chIdx] = impedanceResult;
                ...
                if (update_ui && impedanceResult != int.MaxValue)
                {
                    ...
                    else
                    {
                        ...
                        Invoke(new Action(() =>
                        {
                            ...
                            if (impedanceResult <= OpenBCISensorSettings.Settings.SignalQuality_ImpedanceGoodMaxThreshold)
                                impedanceResultColor = SelectColorFromStatus(SignalStatus.SIGNAL_OK);
                            else if (impedanceResult > OpenBCISensorSettings.Settings.SignalQuality_ImpedanceGoodMaxThreshold &&
                                impedanceResult <= OpenBCISensorSettings.Settings.SignalQuality_ImpedanceOkMaxThreshold)
                                impedanceResultColor = SelectColorFromStatus(SignalStatus.SIGNAL_ACCEPTABLE);
                            else if (impedanceResult > OpenBCISensorSettings.Settings.SignalQuality_ImpedanceOkMaxThreshold)
        ...
        public bool updateSignalQualityResult(int chIdx)
        {
            ...
            try
            {
                int currentRailingPercentage = OpenBCISensorSettings.Settings.SignalQuality_LastRailingValues[chIdx];
                int currentImpedanceResult = OpenBCISensorSettings.Settings.SignalQuality_LastImpedanceValues[chIdx];
                ...
                if (currentRailingPercentage != int.MaxValue && currentImpedanceResult != int.MaxValue)
                {
                    ...
                    if (currentRailingPercentage <= OpenBCISensorSettings.Settings.SignalQuality_RailingGoodMaxThreshold &&
                        currentImpedanceResult <= OpenBCISensorSettings.Settings.SignalQuality_ImpedanceGoodMaxThreshold)
                        overallSignalQualityStatus = SignalStatus.SIGNAL_OK;
                    else if (currentRailingPercentage <= OpenBCISensorSettings.Settings.SignalQuality_RailingOkMaxThreshold &&
                        currentImpedanceResult <= OpenBCISensorSettings.Settings.SignalQuality_ImpedanceOkMaxThreshold)
