...
namespace ACAT.Extensions.BCI.Sensors.OpenBCISensor
{
    ...
    public class OpenBCIDeviceTester
    {
        ...
        public bool OpenBCIDeviceAvailable
        {
            get { 
                ...
                {
                    return DAQ_OpenBCI.IsOpenBCIDeviceAvailable(); // Check if OpenBCI device is available
                }
            }
        }
        ...
        public OpenBCIDeviceTester()
        {
            try
            {
                _Testing_useSensor = BCISensorSettings.Settings.Testing_UseSensor;
                _Testing_BCIOnboardingIgnoreOpticalSensorChecks = BCISensorSettings.Settings.Testing_BCIOnboardingIgnoreOpticalSensorChecks;
            }
            catch (Exception ex)
            {
                Log.Exception(ex);
        ...
        /// <summary>
        /// Do all OpenBCIDeviceTester initialization - this is done in a separate function instead of the constructor
        ...
        private void startSignalQualityTestingState(DeviceTestingState newDeviceTestingState)
        {
            ...
            if (newDeviceTestingState == DeviceTestingState.BCISignalCheckStartRequired)
            {
                ...
                long timestampPrevImpedanceTest = BCISensorSettings.Settings.SignalQuality_TimeOfLastImpedanceCheck;
                ...
                double maxTimeMins = (double)BCISensorSettings.Settings.SignalQuality_MaxTimeMinsElapsedSinceLastImpedanceCheck​;
                ...
                bool userPassedLastSignalQualityCheck = BCISensorSettings.Settings.SignalQuality_PassedLastOverallQualityCheck;
            ...
            else if (newDeviceTestingState == DeviceTestingState.PromptFilterSettings)
            {
                ...
                bool showFilterSettings = BCISensorSettings.Settings.DAQ_ShowFilterSettings;
        ...
        private void finishSignalQualityTestingState(DeviceTestingState currentDeviceTestingState)
        {
            ...
            else if (currentDeviceTestingState == DeviceTestingState.BCISignalCheckStartPrompt)
            {
                ...
                {
                    // If no recheck needed, transition out of OpenBCIDeviceTester all together (everything successfully completed)
                    ...
                    BCISensorSettings.Settings.SignalControl_RecheckNeeded = false;
                ...
                {
                    // Save SignalQuality_RecheckNeeded
                    BCISensorSettings.Settings.SignalControl_RecheckNeeded = true;
                    BCISensorSettings.Save();
            ...
            else if (currentDeviceTestingState == DeviceTestingState.PromptFilterSettings)
            {
                ...
                if (_mainForm._userControlPromptBCIFIlterSettings.checkBoxConfirm60HzCountry.Checked)
                {
                    ...
                    BCISensorSettings.Settings.DAQ_NotchFilterIdx = 2;
                }
                else
                {
                    BCISensorSettings.Settings.DAQ_NotchFilterIdx = 1;
                }

                if (_mainForm._userControlPromptBCIFIlterSettings.checkBoxDontShowStartup.Checked)
                {
                    BCISensorSettings.Settings.DAQ_ShowFilterSettings = false;
                }
                else
                {
                    BCISensorSettings.Settings.DAQ_ShowFilterSettings = true;
                }
            ...
            else if (currentDeviceTestingState == DeviceTestingState.BCISignalCheck)
            {
                ...
                if (BCISensorSettings.Settings.Testing_IgnoreSignalTestResultDuringOnboarding)
                {
                ...
                int chnIdx = 0;
                while (chnIdx < BCISensorSettings.Settings.DAQ_NumEEGChannels)
                {
                    channelNames[chnIdx] = UserControlBCISignalCheck._eegChannels[chnIdx]._electrodeName;
                    enabledChannels[chnIdx] = BCISensorSettings.Settings.GetClassifier_EnableChannel(chnIdx);
                ...
                if (exitBCIOnboarding)
                {
                    ...
                    BCISensorSettings.Save(); // Save settings
        ...
        public async Task TriggerBoxHighlighting() // Original function
        {
            ...
            while (!_endTasks && !_endTriggerBoxTask)
            {
                if (_requestTestTriggerBox && !startTest)
                {
                    ...
                    Log.Debug("OPTSEN: Setting lux threshold to " + BCISensorSettings.Settings.DAQ_OpticalSensorLuxThreshold);
                    OpticalSensorComm.SetLuxThreshold(BCISensorSettings.Settings.DAQ_OpticalSensorLuxThreshold);
