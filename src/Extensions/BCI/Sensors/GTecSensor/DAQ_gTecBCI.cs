...
namespace ACAT.Extensions.BCI.Sensors.GTecSensor
{
    public class DAQ_gTecBCI
    {
        ...
        public BCIGtecSettings BCIGtecActuatorSettings;

        public DAQ_gTecBCI()
        {
            // Initialize status
            status = BoardStatus.BOARD_STANDBY;
            sensorStatus = ExitCode.STATUS_OK;
            // Initialize buffers
            _bufferSignalStatus = null;
        ...
        {
            SignalControl_WindowDurationForVrmsMeaseurment = BCIGtecActuatorSettings.SignalControl_WindowDurationForVrmsMeaseurment;
            SignalControl_MinDutyCycleToPassTriggerTest = BCIGtecActuatorSettings.TriggerTest_MinDutyCycleToPassTriggerTest;
            ...
            Log.Debug("Sensor set to " + BCIGtecActuatorSettings.DAQ_NumEEGChannels + " channels. SensorID: " + BCISettingsFixed.DAQ_SensorId + " , Downsample rate: " + BCISettingsFixed.DimReduct_DownsampleRate +
            ...
            saveDataToFile = BCIGtecActuatorSettings.DAQ_SaveToFileFlag;
            frontendFilterIdx = BCIGtecActuatorSettings.DAQ_FrontendFilterIdx;
            notchFilterIdx = BCIGtecActuatorSettings.DAQ_NotchFilterIdx;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (sensorConnected)
                    {
                        ...
                        BCIGtecActuatorSettings.GTecDeviceName = serial_number;
                        BCIGtecActuatorSettings.Save();
                ...
                if (status == BoardStatus.BOARD_ACQUIRINGDATA)
                {
                    ...
                    if (rawData != null && rawData.Length > 0)
                    {
                        ...
                        if (saveDataToFile && FileWriterObj != null && FileWriterObj.isFileOpened)
                        {
                            ...
                            if (BCIGtecActuatorSettings.DAQ_SaveAditionalFileWithRawData)
        ...
        public async Task<bool> connectionTestAsync()
        {
            ...
            if (string.IsNullOrEmpty(BCIGtecActuatorSettings.GTecDeviceName))
            {
                Dictionary<String, object> eventParams = new Dictionary<String, object>
                {
            ...
            else
            {
                ...
                return await Task.Run(() =>
                {
                    try
                    {
                        Log.Debug($"Selected device: {BCIGtecActuatorSettings.GTecDeviceName}, trying to connect...");
                        using (Unicorn device = new Unicorn(BCIGtecActuatorSettings.GTecDeviceName))
                        {
                            device.Dispose();
