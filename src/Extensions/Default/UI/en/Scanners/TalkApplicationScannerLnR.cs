...
{
    [DescriptorAttribute("1862558B-FE3A-45A9-AF19-3D712EE513DE",
                        "TalkApplicationScannerLnR",
                        "Talk application main window for Listen and Respond")]
    public partial class TalkApplicationScannerLnR : Form, IScannerPanel, ISupportsStatusBar
    {
        ...
        private bool _lnRResponsesDisplayed = true;

        /// <summary>
        /// Initializes a new instance of the class.
        /// </summary>
        public TalkApplicationScannerLnR()
        {
        ...
        public bool Initialize(StartupArg startupArg)
        {
            ...
            _scannerCommon.UserControlManager.AddUserControlByKeyOrName(panelSentencePrediction, "lnrResponses", "SentencePredictionLnRUserControl");
            ...
            Context.AppAgentMgr.EvtEditingModeSet += AppAgentMgr_EvtEditingModeSet;
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            {
                _activeDialogSenseAgent.DialogTranscript.EvtTurnAdded -= DialogTranscript_EvtTurnAdded;
                _activeDialogSenseAgent.DialogTranscript.EvtTranscriptCleared -= DialogTranscript_EvtTranscriptCleared;
                _activeDialogSenseAgent.EvtMessageReceived += ActiveDialogSenseAgent_EvtMessageReceived;
            }
        ...
        private void addTextBoxUserControl(UserControl userControl)
        {
            ...
            if (userControl is ITalkWindowTextBox)
            {
                ...
                if (_textBoxTalkWindow != null)
                {
                    ...
                    _textBoxTalkWindow.KeyUp += TextBox_KeyUp;
        ...
        private void textToSpeech(String text)
        {
            if (!String.IsNullOrEmpty(text))
            {
                ...
                if (!isNullDialogSenseAgent())
                {
                    _activeDialogSenseAgent.DialogTranscript.AddTurn(new DialogTurn(DialogTurnType.User, text));
                }
        ...
        private class CommandHandler : RunCommandHandler
        {
            ...
            public override bool Execute(ref bool handled)
            {
                ...
                var form = Dispatcher.Scanner.Form as TalkApplicationScannerLnR;
        ...
        private void DialogTranscript_EvtTranscriptCleared()
        {
            Windows.SetText(richTextDialogHistory, String.Empty);
        }

        private void DialogTranscript_EvtTurnAdded(DialogTurn turn)
        {
            Log.Debug("DUMDUM: Turn added" + turn.Text + "  " + turn.TurnType.ToString());

            switch(turn.TurnType)
            {
                case DialogTurnType.User:
                    addDialogText(turn.Text, HorizontalAlignment.Right, richTextDialogHistory.Font, Color.White);
                    break;

                case DialogTurnType.Other:
                    var color = Color.FromArgb(0xB1, 0xFF, 0xFF);
                    var font = new Font(richTextDialogHistory.Font, FontStyle.Italic | FontStyle.Bold);
                    addDialogText(turn.Text, HorizontalAlignment.Left, font, color);
                    break;
            }
        }

        private void addDialogText(String text, HorizontalAlignment align, Font font, Color color)
        {
            Invoke(new MethodInvoker(delegate
            {
                richTextDialogHistory.SelectionAlignment = align;
                richTextDialogHistory.SelectionFont = font;
                richTextDialogHistory.SelectionColor = color;
                richTextDialogHistory.AppendText(text + Environment.NewLine);
                richTextDialogHistory.AppendText(Environment.NewLine);
                richTextDialogHistory.ScrollToCaret();
            }));
        }
        ...
        private void AppAgentMgr_EvtEditingModeSet(EditingMode oldMode, EditingMode newMode)
        {
            displaySentencePrediction();
            
        }

        private void labelCurrentTypingMode_Click(object sender, EventArgs e)
        {
            displayLnRResponses();

            Context.AppDialogSenseManager.TriggerRefreshResponses(this);
        }

        private void displaySentencePrediction()
        {
            if (_lnRResponsesDisplayed)
            {
                _lnRResponsesDisplayed = false;
                Invoke(new MethodInvoker(delegate
                {
                    //_scannerCommon.UserControlManager.AddUserControlByKeyOrName(panelSentencePrediction, "sentencePrediction", "SentencePredictionUserControl");
                    _scannerCommon.UserControlManager.PushUserControlByKeyOrName(panelSentencePrediction, "sentencePrediction", "SentencePredictionUserControl", true);
                    _scannerCommon.UserControlManager.StartTopLevelAnimation();
                }));
            }
        }

        private void displayLnRResponses()
        {
            if (!_lnRResponsesDisplayed)
            {
                _lnRResponsesDisplayed = true;
                Invoke(new MethodInvoker(delegate
                {
                    //_scannerCommon.UserControlManager.AddUserControlByKeyOrName(panelSentencePrediction, "lnrResponses", "SentencePredictionLnRUserControl");
                    _scannerCommon.UserControlManager.PushUserControlByKeyOrName(panelSentencePrediction, "lnrResponses", "SentencePredictionLnRUserControl", true);
                    _scannerCommon.UserControlManager.StartTopLevelAnimation();
        ...
        private void ActiveDialogSenseAgent_EvtMessageReceived(object sender, string message)
        {
            displayLnRResponses();

            Context.AppDialogSenseManager.TriggerRefreshKeywords();
        ...
        private void AppDialogSenseManager_EvtRefreshResponses(object sender, string keyword)
        {
            if (sender != this)
            {
                ...
                Context.AppDialogSenseManager.TriggerRefreshResponses(this, keyword);
            }
        }
