...
using ACAT.Extensions.BCI.Common.EEGUtils;
...
namespace ACAT.Extensions.BCI.Sensors.OpenBCISensor
{
    public class DAQ_OpenBCI
    {
        ...
        public static OpenBCISensorSettings BCISensorSettings = new OpenBCISensorSettings();
        ...
        {
            SignalControl_WindowDurationForVrmsMeaseurment = BCISensorSettings.Settings.SignalControl_WindowDurationForVrmsMeaseurment;
            SignalControl_MinDutyCycleToPassTriggerTest = BCISensorSettings.Settings.TriggerTest_MinDutyCycleToPassTriggerTest;
            ...
            switch (BCISensorSettings.Settings.DAQ_NumEEGChannels)
            {
                case 8:
                ...
                default:
                    BCISensorSettings.Settings.DAQ_NumEEGChannels = 8;
                    BCISettingsFixed.DAQ_SensorId = 0;
                    BCISettingsFixed.DataParser_IdxTriggerSignal_Hw = 16;
                    BCISettingsFixed.DataParser_IdxTriggerSignal_Sw = 24;
                    BCISettingsFixed.DimReduct_DownsampleRate = 2;
            ...
            BCISensorSettings.Save();
            Log.Debug("Sensor set to " + BCISensorSettings.Settings.DAQ_NumEEGChannels + " channels. SensorID: " + BCISettingsFixed.DAQ_SensorId + " , Downsample rate: " + BCISettingsFixed.DimReduct_DownsampleRate +
            ...
            saveDataToFile = BCISensorSettings.Settings.DAQ_SaveToFileFlag;
            frontendFilterIdx = BCISensorSettings.Settings.DAQ_FrontendFilterIdx;
            notchFilterIdx = BCISensorSettings.Settings.DAQ_NotchFilterIdx;
            Log.Debug(" Frontend filter: " + frontendFilterIdx + " Notch filter: " + notchFilterIdx);

            eyesClosedDetectionUseFixThreshold = BCISensorSettings.Settings.EyesClosed_UseFixThreshold;
            if (eyesClosedDetectionUseFixThreshold)
                eyesClosedDetectionThreshold = BCISensorSettings.Settings.EyesClosed_FixThreshold_Threshold;
            else
                eyesClosedDetectionThreshold = BCISensorSettings.Settings.EyesClosed_AdaptiveThreshold;
            eyesClosed_WindowDuration = BCISensorSettings.Settings.EyesClosed_WindowDuration;
        ...
        public static ExitCodes getUsbDongleConnected(String port = null)
        {
            try
            {
                ...
                if (port == null || port == "")
                {
                    ...
                    port = BCISensorSettings.Settings.DAQ_ComPort;
                }
                // Test port
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (sensorConnected)
                    {
                        ...
                        BCISensorSettings.Settings.DAQ_ComPort = serialPort;
                        BCISensorSettings.Save();
                ...
                if (status == BoardStatus.BOARD_ACQUIRINGDATA)
                {
                    ...
                    if (rawData != null && rawData.Length > 0)
                    {
                        ...
                        if (saveDataToFile && FileWriterObj != null && FileWriterObj.isFileOpened)
                        {
                            ...
                            if (BCISensorSettings.Settings.DAQ_SaveAditionalFileWithRawData)
        ...
        private static bool cytonIsDaisyAttached(String comPort)
        {
            ...
            try
            {
                ...
                if (receivedResponse == true)
                {
                    ...
                    {
                        BCISensorSettings.Settings.DAQ_NumEEGChannels = 16;
                    ...
                    Log.Debug("cytonIsDaisyAttached | Received a valid response from cyton board | DAQ_NumEEGChannels: "+
                        BCISensorSettings.Settings.DAQ_NumEEGChannels.ToString());
        ...
        public static bool IsOpenBCIDeviceAvailable()
        {
            ExitCodes status = getUsbDongleConnected(); // Check if USB dongle is connected

            if (status != ExitCodes.STATUS_OK)
            {
                Log.Debug("OpenBCI device is not available. Status: " + status.ToString());
                return false;
            }

            if (DeviceObj != null && DeviceObj.is_prepared())
            {
                Log.Debug("OpenBCI device is available and connected");
                return true;
            }
            else
            {
                Log.Debug("OpenBCI device is not available or not connected");
                return false;
            }
        }
    }
