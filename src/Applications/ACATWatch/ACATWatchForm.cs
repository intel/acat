...
//
// The main Form for ACAT Watcher. Opens a Named Pipe server to
// receive requests from ACAT to set focus to the active window.
//
////////////////////////////////////////////////////////////////////////////
...
namespace ACAT.Applications.ACATWatch
{
    public partial class ACATWatchForm : Form
    {
        ...
        public ACATWatchForm()
        {
            ...
            this.Load += new EventHandler(LauncherForm_Load);
        ...
        public static void ForceWindowIntoForeground(IntPtr window)
        {
            ...
            IntPtr activeWindow = User32Interop.GetForegroundWindow();
            uint activeThread = User32Interop.GetWindowThreadProcessId(activeWindow, out uint _);
            uint windowThread = User32Interop.GetWindowThreadProcessId(window, out uint _);

            if (currentThread != activeThread)
                User32Interop.AttachThreadInput(currentThread, activeThread, true);
            if (windowThread != currentThread)
                User32Interop.AttachThreadInput(windowThread, currentThread, true);
            ...
            if (currentThread != activeThread)
                User32Interop.AttachThreadInput(currentThread, activeThread, false);
            if (windowThread != currentThread)
                User32Interop.AttachThreadInput(windowThread, currentThread, false);
        }

        public void FocusWindow(IntPtr focusOnWindowHandle)
        {
        ...
        private void _pipeServer_MessageReceived(object sender, MessageReceivedEventArgs e)
        {
            if (String.Compare(e.Message, "quit", true) == 0)
            {
        ...
        private void LauncherForm_Load(object sender, EventArgs e)
        {
            ...
            Windows.ShowInTaskbar(this, false);
        ...
        private void PipesMessageHandler(string message)
        {
            try
            {
                ...
                else
                {
                    try
                    {
                        if (String.Compare(message, "quit", true) == 0)
                        {
                            Windows.CloseForm(this);
                            return;
                        }
